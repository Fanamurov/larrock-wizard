<div class="ibox-content uk-margin-large-bottom">
    ...
    <form action="/admin/wizard/storeConfig" method="post" class="uk-form">
        {!! csrf_field() !!}
        <table class="uk-table uk-margin-remove-bottom uk-margin-remove-top table-config">
            <thead>
            <tr>
                    ...
                    @if( !empty($colomn_name))
                        <td>
                            @if($colomn_name === 'foto')
                                ...
                                <input class="uk-input uk-form-small" type="text" disabled value="[system]">
                            @else
                                <select class="wizard-db-colomns uk-select uk-form-small" name="db[]">
                    ...
                    @if( !empty($colomn_name))
                        <td>
                            @if($colomn_name === 'foto')
                                ...
                                <input type="text" disabled value="[system]" class="uk-width-1-1 uk-input uk-form-small">
                            @else
                                <input class="uk-width-1-1 uk-input uk-form-small" name="slug[]" type="text" value="@if(isset($rows[$colomn_name])) {{ $rows[$colomn_name]['slug'] }} @endif" placeholder="-- Не назначено --">
                            @endif
                        </td>
                    @endif
                @endforeach
                    ...
                    @if( !empty($colomn_name))
                        <td>
                            @if($colomn_name === 'foto')
                                ...
                                <input type="text" disabled value="[system]" class="uk-width-1-1 uk-input uk-form-small">
                            @else
                                <select class="wizard-db-colomns uk-width-1-1 uk-select uk-form-small" name="template[]">
                                    <option value="">-- Не назначено --</option>
                    ...
                    @if( !empty($colomn_name))
                        <td>
                            @if($colomn_name === 'foto')
                                ...
                                <input type="text" disabled value="[system]" class="uk-width-1-1 uk-input uk-form-small">
                            @else
                                <select class="wizard-db-colomns uk-width-1-1 uk-select uk-form-small" name="filters[]">
                                    <option value="">-- Не назначено --</option>
                    ...
                    @if( !empty($colomn_name))
                        <td>
                            @if($colomn_name === 'foto')
                                ...
                                <input type="text" disabled value="[system]" class="uk-width-1-1 uk-input uk-form-small">
                            @else
                                <select class="wizard-db-colomns uk-width-1-1 uk-select uk-form-small" name="admin[]">
                                    <option value="">-- Не назначено --</option>
            ...
            <tr>
                <td colspan="{!! count($data->first())+1 !!}">
                    <button type="submit" class="uk-button uk-button-primary uk-button-large">Сохранить настройки импорта</button>
                </td>
            </tr>
            </tbody>
        </table>
...
<div class="ibox-content">
    ...
    <progress class="uk-progress" value="0" max="{{ count($data) }}"></progress>

    @foreach($rows as $key => $row)
        @if( !empty($key) && $key !== 'foto' && !$row['db'])
    ...
    @foreach($data as $data_key => $data_value)
        @if( !empty($data_value['naimenovanie']))
            <form action="/admin/wizard/importrow" method="post" class="import_row uk-form">
                {!! csrf_field() !!}
                <table class="uk-table uk-margin-remove-bottom uk-margin-remove-top">
                    @if($loop->first)
                        <thead>
                        @if($loop->first)
                            <tr>
                            ...
                            @if( !empty($colomn_name))
                                ...
                                @if($colomn_name === 'foto')
                                    <td class="uk-position-relative">
                                        <select name="{{ $colomn_name }}" class="cell_value uk-select uk-form-small" style="width: 150px" data-oldvalue="{{ $colomn_value }}"
                                        ...
                                        <button style="position: absolute; top: 4px; right: 3px;" type="button"
                                                class="find_image uk-button uk-button-primary uk-form-small uk-button-small"
                                                data-coordinate="@lang('excelcells.'.$loop->iteration){{ $data_key+2 }}" data-sheet="{{ $sheet }}">+</button>
                                    </td>
                                @elseif(isset($rows[$colomn_name]) && empty($rows[$colomn_name]['db']))
                                    <td><input data-oldvalue="{{ $colomn_value }}" data-coordinate="@lang('excelcells.'.$loop->iteration){{ $data_key+2 }}" data-sheet="{{ $sheet }}"
                                               name="{{ $colomn_name }}" class="cell_value uk-input uk-form-small" type="text" value="{{ $colomn_value }}" data-uk-tooltip title="{{ $colomn_value }}"></td>
                                @else
                                    <td><input data-oldvalue="{{ $colomn_value }}" data-coordinate="@lang('excelcells.'.$loop->iteration){{ $data_key+2 }}" data-sheet="{{ $sheet }}"
                                               name="{{ $rows[$colomn_name]['db'] }}" class="cell_value uk-input uk-form-small"
